...
shinyUI(bslib::page_navbar(
  title = "PhysioSim v0.0.1",
  theme = bs_theme(version = 5, bootswatch = "cerulean", font_scale = 0.9), # themes can be found in https://bootswatch.com
                   ...
                   tabPanel("Inputs/Simulation",
                            sidebarLayout(
                              sidebarPanel(
                                shinyjs::useShinyjs(),
                                h5("Define model inputs for PBPK simulations"),
                                ...
                                fluidRow(
                                  column(12,
                                         numericInput("inf_dur",
                                                      label = 'Infusion duration (h)',
                                                      value = 0.5, step=0.1, min=0, max=6)
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                  sliderInput("daily_admin",
                                              label = 'Daily administrations',
                                              min = 1, max = 4,
                                              value = 1, step=1),
                                  )
                                ),
                                ...
                                fluidRow(
                                  ...
                                  column(12,
                                         numericInput("dose", label = "Dose Amount", value = 10)
                                  )
                                ),
                                hr(),
                                ...
                                fluidRow(
                                  column(12,
                                         numericInput('MW', label = 'mw (g/Mol)', value = 500)
                                         # "molecular weight: it is used to calculate the diffusion coefficient in the Noyes-Whitney model describing drug dissolution in the gastro-intestinal lumen"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         radioButtons("type", # "type: it influences how the drug solubility in gastro-intestinal tract is calculated from the drug intrinsic solubility"
                                                            label = 'Type', 
                                                            choices = list("Neutral" = 1, 
                                                                           "Acid" = 2, # monoprotic acid
                                                                           "Base" = 3), # monoprotic base
                                                            selected = 1)
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('logPow', label = 'logPow', value = 2)
                                         # "logarithm of the octanole to water partition coefficient: it is used to calculate the tissue to plasma partition coefficients"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('fup', label = 'fup', value = 0.8,min=0,max=1)
                                         # "fraction unbound in plasma: used to calculate the tissue to plasma partition coefficients and the fraction unbound in tissues"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('BP', label = 'BP', value = 0.8)
                                         # "blood to plasma ratio: it is used to calculate the plasma concentration from the blood one and to calculate the tissue to plasma partition coefficient"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('pKa', label = 'pKa', value = 0.8)
                                         # "acid dissociation constant: it is used to derive the solubility in the gastro-intestinal tract from the intrinsic solubility"
                                  )
                                ),
                                hr(),
                                ...
                                fluidRow(
                                  column(12,
                                         numericInput('r', label = 'Particle Radius (um)', value = 25)
                                         # "particle radius of the formulation: it is used in the Noyes-Whitney model to derive the rate constant of drug dissolution in the gastro-intestinal tract"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('rho', label = 'Density of Formulation (g/L)', value = 1000)
                                         # "density of the formulation: it is used to calculate the hydrodynamic radius of the diffusing drug and it is used in the Noyes-Whitney model to derive the rate constant of drug dissolution in the gastro-intestinal tract"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('Csint', label = 'Intrinisic Solubility (mg/L)', value = 100)
                                         # "intrinsic solubility: is the solubility of the compound in its free acid or free base form; it is used to calculate the rate constant of drug dissolution in the Noyes-Whitney model"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('Peff_caco2', label = 'Effective Permeability (10^-4 cm/s)', value = 2)
                                         # "effective permeability: it is used to calculate the absorption rate constant in the small intestine compartments"
                                  )
                                ),
                                hr(),
                                ...
                                fluidRow(
                                  column(12,
                                         numericInput('Clh', label = 'Hepatic Intrinsic Clearance (L/h)', value = 10)
                                         # "hepatic intrinsic clearance: it can be defined as a proportionality constant between the drug elimination rate from the liver and the unbound concentration of drug in the liver"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('Clr', label = 'Renal Intrinsic Clearance (L/h)', value = 10)
                                         # "renal intrinsic clearance: it can be defined as a proportionality constant between the drug elimination rate from the kidneys and the unbound concentration of drug in the kidneys"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         checkboxInput("GFR_flag", "add GFR", value = FALSE, width = NULL)
                                         # "Glomerular filtration rate: it is the flow rate of filtered fluid through the kidneys"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         numericInput('Clent', label = 'Enterocyte Clearance (L/h)', value = 0)
                                         # "enterocyte clearance: it can be defined as a proportionality constant between the drug elimination rate from the enterocytes and the whole concentration of drug in the enterocytes"
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                         radioButtons("PCM",  # "partition coefficient: for a given organ, it is defined as the ratio between the drug concentration in the tissue and in the organ outgoing blood; it is used to describe the distribution of the drugs into the various organs"
                                                            label = 'Partition Coefficient Method', 
                                                            choices = list("Poulin & Theil" = "P&T", 
                                                                           "Schmitt" = "Schmitt",
                                                                           "Rowland & Rodgers" = "R&R",
                                                                           "Berezhkhovsky" = "bere",
                                                                           "PK-Sim Standard" = "PK-Sim"),
                                                            selected = "P&T")
                                  )
                                ),
                                hr(),
                                ...
                                fluidRow(
                                  ...
                                  column(12,
                                         br(),
                                         actionButton("uploadDrugParam", "Upload", width='100%'),
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                ...
                                fluidRow(
                                  ...
                                  column(12,
                                         br(),
                                         actionButton("scalex", "scale x", width = "100%"),
                                  )
                                ),
                                fluidRow(
                                  column(12,
                                  ...
                                  column(12,
                                         br(),
                                         actionButton("scaley", "scale y", width = "100%"),
                                  )
                                ),
                                br(),
                                actionButton("clearPlot", "Clear plots", width='100%'),
                                hr(),
                                helpText("Press the button to run the simulation"),
                                actionButton("runButton", "Run simulation", width='100%', class = "btn-primary"),
                                hr(),
                                ...
                                actionButton("nca", "Perform NCA", width='100%', class = "btn-info"),
                                hr(),
                                helpText("Download data & images"),
                                actionButton("download_menu", "Download menu", width='100%', class = "btn-secondary"),
                                hr(),
                                helpText("Press the button to reset to default values"),
                                actionButton("resetButton", "Reset", width='100%', class = "btn-warning")
                              ),
                              mainPanel(
                                fluidRow(
                                ),
