...
calcKp_Berez <- function(logP, pKa, fup, BP=1, type=1, dat){
  ...
  dat_all <- dat %>% filter(!tissue %in% c("Plasma","Fat","RBCs"))
  ...
  Vwt <- dat2$f_water[dat2$tissue != "Fat"]
  Vwad <- dat2$f_water[dat2$tissue == "Fat"]
  Vnlt <- dat2$f_n_l[dat2$tissue != "Fat"]
  Vnlad <- dat2$f_n_l[dat2$tissue == "Fat"]
  Vpht <- dat2$f_pl[dat2$tissue != "Fat"]
  Vphad <- dat2$f_pl[dat2$tissue == "Fat"]
  
  fut <- 1/(1+((1-fup)/fup)*0.5)
  
  pH <- dat$pH[dat$tissue == "Fat"]
  ...
  nms_all <- dat_all$tissue %>% tolower()
